#' Run tidy knn imputation and return results
#'
#' Run tidy knn imputation and return results
#'
...
run_tidy_knn <- function(data_train_X_miss, data_test_X_miss, y_train, y_test,
  ...
  rec_obj <- recipes::recipe(~ ., data = data_train_X_miss)

  imp_model <- rec_obj %>%
    recipes::step_impute_knn(all_of(vars_to_impute),
